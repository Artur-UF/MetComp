...
def simula(xa, xb, va, vb, r, tf, dt):
    ...
    pa = Particle(r, [xa, 0], va, fa(xa, xb), ma, 'blue')
    pb = Particle(r, [xb, 0], vb, fb(xa, xb), mb, 'red')
    parts = [pa, pb]
    fs = [fa, fb]
    iter = zip(parts, fs)
    for p, f in iter:
        p.vel += (f(pa.pos[0], pb.pos[0]) * dt) / 2
    t = np.arange(0, tf, dt)
    for ti in t:
        for p in parts:
        ...
        '''
        for p, f in iter:
            p.forc = f(pa.pos[0], pb.pos[0])
        '''
        pa.forc = fa(pa.pos[0], pb.pos[0])
        pb.forc = fb(pa.pos[0], pb.pos[0])
